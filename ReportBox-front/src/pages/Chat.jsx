        ...
        if (!storedUserId) {
          ...
          try {
            ...
            if (userObj && userObj.id) {
              ...
              console.log('Using malformed JSON as plain userId:', actualUserId);
            }
          } catch (parseError) {
    ...
    try {
      ...
      if (data.success && data.data && data.data.history && data.data.history.length > 0) {
        ...
        setMessages([{
          id: 'welcome',
          type: 'bot',
          content: "Welcome to SEVAK Legal Assistant! I'm here to help you with legal questions and guidance. How can I assist you today?",
          timestamp: new Date(),
          status: 'delivered'
        }]);
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = `HTTP ${response.status}: ${errorText || 'Failed to get response'}`;
        }
    ...
    return (
      <div className="h-screen w-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 flex items-center justify-center">
        <div className="text-white text-center bg-black/30 backdrop-blur-xl border border-white/20 rounded-3xl p-8 max-w-md mx-4">
          ...
          <p className="text-slate-300 mb-6">You need to be logged in to access SEVAK Legal Assistant.</p>
          <a 
            href="/login" 
    ...
    return (
      <div className="h-screen w-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 flex items-center justify-center">
        <div className="text-white text-center">
          ...
          {userId && <p className="text-sm text-slate-400 mt-2">User ID: {userId}</p>}
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">Legal AI • Always available</p>
            </div>
          </div>
        </div>

        <div className="flex items-center space-x-2">
          <span className="text-xs text-slate-500">ID: {userId?.slice(-8)}</span>
          ...
          >
            <div className={`flex items-end space-x-3 max-w-[85%] ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
              ...
              <div className="relative group">
                <div className={`rounded-3xl px-6 py-4 relative shadow-lg ${
                  message.type === 'user'
      ...
      <div className="bg-black/30 backdrop-blur-xl border-t border-white/10 p-4 relative z-10 flex-shrink-0">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {legalCategories.map((category, index) => (
            <button
              key={index}
              onClick={() => setInputMessage(`I need help with ${category.toLowerCase()}`)}
          ...
          <button
            ...
            className={`p-3 rounded-2xl transition-all duration-300 flex-shrink-0 ${
              isRecording 
                ...
                : 'bg-white/10 hover:bg-white/20 border border-white/20 text-slate-400 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed'
            }`}
